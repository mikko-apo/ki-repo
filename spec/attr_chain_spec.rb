...
describe "attr_chain" do
  ...
  it ":valid should warn about invalid values" do
    ...
    f=Foo.new
    f.bar("a").bar.should == "a"
    f.bar(1).bar.should == 1
    f.bar("test").bar.should == "test"
    Foo.new.bar(f.bar(1)).bar.bar.should == 1
  ...
  it "should accept long configuration syntax" do
    class Foo
      ...
      attr_chain :valid_short, :valid => lambda { |s| raise "valid_test has been set already" if @valid_test }
      attr_chain :valid_test
    end
    f = Foo.new
    f.default.should == "b"
    f.immutable(1).immutable.should == 1
    ...
    f.valid_short("a").valid_short.should == "a"
    f.valid_test(1)
  ...
  it "should convert values" do
    ...
    f = Foo.new
    f.converted(1).converted.should == 2
    f.converted_2(1).converted_2.should == 3
  end
  ...
  it "should provide accessor to edit other than local fields" do
    ...
    f = Foo.new
    f.email.should == "test@email.fi"
    f.cached_data.keys.should == [:email]
    f.email = "test"
    f.email.should == "test"
    lambda { f.name }.should raise_error(RuntimeError, "'name' has not been set")
    f.name="foo"
    f.cached_data.should == {:email=>"test", :name=>"foo"}
  end
